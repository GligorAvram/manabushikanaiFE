...
export class WriterApiService extends ApiService {
  ...
  submitTranslationForParagraph(
    paragraph: CreateParagraphTranslationDto,
  ): Observable<ApiResult<ParagraphDto>> {
    return this.patch<{ id: string; sentences: CreateSentenceDto[] }, ParagraphDto>(
      apiRoutes.writer.stories.translations,
      { id: paragraph.id!, sentences: paragraph.sentences! },
      this.writerStore.onSentenceTranslationAdded.bind(this.writerStore),
    );
  }

  submitDictionaryWord(
    word: CreateDictionaryWordDto,
  ): Observable<ApiResult<DictionaryWordDto>> {
    return this.post<DictionaryWordDto, CreateDictionaryWordDto>(
      apiRoutes.writer.stories.dictionary,
      word,
    );
  }
}